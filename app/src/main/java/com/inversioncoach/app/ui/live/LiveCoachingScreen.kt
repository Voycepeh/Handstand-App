...
fun LiveCoachingScreen(drillType: DrillType, onStop: () -> Unit) {
    ...
    val settingsFlow = remember { MutableStateFlow(UserSettings()) }
    ...
    Box(Modifier.fillMaxSize()) {
        AndroidView(
            ...
            factory = { ctx ->
                PreviewView(ctx).apply {
                    post { cameraManager.bind(lifecycleOwner, this, analyzer) }
                }
            },
        )
        OverlayRenderer(
        ...
        Row(
            ...
            Button(onClick = vm::toggleRecording) { Text(if (uiState.isRecording) "Stop Rec" else "Record") }
